<script>
    ...
    document.querySelectorAll('.chart-btn').forEach(btn => {
        btn.classList.add('px-3', 'py-1.5', 'rounded-full', 'text-xs', 'font-semibold', 'border', 'border-gray-200', 'transition');
        btn.addEventListener('click', () => {
            const chart = btn.dataset.chart;
            const period = btn.dataset.period;
            document.querySelectorAll(`[data-chart="${chart}"]`).forEach(b => b.classList.remove('bg-orange-50', 'text-[#f15a24]'));
            btn.classList.add('bg-orange-50', 'text-[#f15a24]');
            loadChart(chart, period);
        });
    });

    ['users', 'articles', 'popular'].forEach(t => loadChart(t, 'daily'));
</script>