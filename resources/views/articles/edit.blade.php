...
@section('content')
    <main class="max-w-6xl mx-auto px-4 py-12 font-['Plus_Jakarta_Sans']">
        <div class="mb-10">
            <h1 class="text-2xl font-bold text-[#1a1c2e] uppercase tracking-tight">Edit Artikel</h1>
            <p class="text-sm text-gray-400">Perbarui ide dan inspirasimu hari ini.</p>
        </div>

        <form method="POST" action="{{ route('articles.update', $article->id) }}" enctype="multipart/form-data" id="article-form"
            class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div
                ...
                <div id="placeholder-text" class="text-center {{ $article->image ? 'hidden' : '' }}">
                    <i class="fas fa-image text-3xl text-gray-300 mb-2"></i>
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Klik untuk ganti gambar</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div class="md:col-span-3">
                    <label class="text-[11px] font-bold text-gray-500 uppercase block mb-2 ml-1">Judul Artikel</label>
                    <input type="text" name="title" value="{{ old('title', $article->title) }}" required placeholder="Masukkan judul..."
                        class="w-full px-5 py-3 rounded-xl border border-gray-200 focus:border-[#f15a24] outline-none bg-white text-sm font-semibold">
                </div>

                <div class="relative">
                    <label class="text-[11px] font-bold text-gray-500 uppercase block mb-2 ml-1">Kategori</label>
            ...
            <div class="space-y-2">
                <label class="text-[11px] font-bold text-gray-500 uppercase block mb-2 ml-1">Konten</label>
                <input type="hidden" name="content" id="content-input">
                <div class="rounded-xl border border-gray-200 overflow-hidden shadow-sm bg-white">
                    {{-- Mengisi editor dengan konten lama --}}
                    <div id="editor" class="min-h-[450px] text-sm leading-relaxed">{!! old('content', $article->content) !!}</div>
                </div>
            </div>

            <div class="flex justify-end gap-4 pt-4">
                <a href="{{ route('articles.my') }}"
                    class="px-6 py-3 text-sm font-bold text-gray-400 hover:text-gray-600 transition">Batal</a>
                <button type="submit"
